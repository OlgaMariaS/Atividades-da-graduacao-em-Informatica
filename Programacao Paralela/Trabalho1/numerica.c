...
#define MAX_COMBINACOES 100000000 // 10^8 (100 milhões)
...
int forca_bruta_numerica_sequencial(const char *senha_alvo, char *senha_encontrada) {
    ...
    printf("[SISTEMA] Aguarde... O progresso sera atualizado a cada 1 milhoes de testes.\n\n");

    for (long long i = 0; i < MAX_COMBINACOES; i++) {
        
        // Formata o número 'i' para ter 8 casas, preenchendo com zeros
        sprintf(tentativa, "%8lld", i);
...
int main() {
    ...
    do {
        ...
        scanf(" %8s", senha_alvo); // O %8s limita a leitura a 8 caracteres
        // 4.444.444
